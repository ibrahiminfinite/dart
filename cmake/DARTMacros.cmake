...
function(dart_generate_export_header)
  ...
  if(UNIX AND NOT __EXPORT_ALL_SYMBOLS_BY_DEFAULT)
    dart_check_compiler_visibility(compiler_supports_visibility)
    if(compiler_supports_visibility)
      target_compile_options(${__TARGET_NAME} PRIVATE -fvisibility=hidden)
    endif()
  endif()

  # Base name
  if(__BASE_NAME)
    set(base_name ${__BASE_NAME})
  else()
    set(base_name "${__TARGET_NAME}")
    string(REPLACE "-" "_" base_name ${base_name})
  endif()
  string(TOUPPER ${base_name} base_name)

  # Set up paths
  set(export_file_name "${__EXPORT_FILE_NAME}")

  set(binary_include_dir "${CMAKE_BINARY_DIR}/${__INCLUDE_DIR}")
  set(binary_export_file_path "${binary_include_dir}/${export_file_name}")

  set(detail_export_file_path "${__INCLUDE_DIR}/detail/${export_file_name}")
  set(binary_detail_export_file_path
    "${CMAKE_BINARY_DIR}/${detail_export_file_path}"
  )
  ...
  generate_export_header(
    ...
    EXPORT_FILE_NAME ${binary_detail_export_file_path}
  )

  # Generate final export header
  file(
    WRITE ${binary_export_file_path}
...
function(dart_add_component)
  ...
  foreach(package ${optional_dependent_packages})
    string(REPLACE "-" "_" package_macro_name ${package})
    if(${package}_FOUND)
      if(NOT DART_SKIP_${package})
        list(APPEND target_compile_definitions_public -DDART_HAVE_${package_macro_name}=1)
        list(APPEND dependent_packages ${package})
      else()
        list(APPEND target_compile_definitions_public -DDART_HAVE_${package_macro_name}=0)
        message(STATUS "Building component <${component_name}> without [${package}] because [DART_SKIP_${package}=ON] is set")
      endif()
    else()
      list(APPEND target_compile_definitions_public -DDART_HAVE_${package_macro_name}=0)
  ...
  if(__GENERATE_EXPORT_HEADER)
    dart_generate_export_header(
      ...
      BASE_NAME ${project_name}_${component_name}
    )
  endif()
...
function(dart_add_component_sub_directory)
  ...
  # Shorter variable names for readability
  set(sub_component_name ${${prefix}_SUB_COMPONENT_NAME})
